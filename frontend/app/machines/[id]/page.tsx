...
export default function MachineDashboard({ params }: { params: { id: string } }) {
  ...
  if (isLoading) return <div className="h-full flex items-center justify-center text-koch-400">Loading Digital Passport...</div>;

  return (
    <div className="flex flex-col h-full overflow-hidden bg-surface">
      {/* Header */}
      <header className="px-8 py-6 border-b border-border bg-surface/80 backdrop-blur-lg shrink-0">
        <div className="flex items-start justify-between">
          <div className="flex items-start gap-4">
            {/* Back Button */}
            <Link href="/machines" className="mt-1 p-2 rounded-lg hover:bg-surface-elevated transition-colors text-koch-400 hover:text-koch-100">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
            </Link>
            
            <div>
              <div className="flex items-center gap-3 mb-1">
                <h1 className="text-3xl font-semibold text-koch-50 tracking-tight">{machineData.name}</h1>
                <span className="px-2.5 py-1 text-[10px] font-bold tracking-wider uppercase bg-accent/10 text-accent rounded-full border border-accent/20">
                  {machineData.status}
                </span>
              </div>
              <div className="flex items-center gap-4 text-sm text-koch-400 font-mono">
          ...
          <button className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-accent text-surface font-medium hover:bg-accent-400 transition-all glow-accent">
      ...
      <main className="flex-1 overflow-y-auto p-8">
        <Tabs defaultValue="timeline" className="h-full flex flex-col">
          ...
          <TabsContent value="timeline" className="flex-1 overflow-y-auto min-h-0 bg-surface-elevated border border-border rounded-xl p-8">
            <h2 className="text-xl font-medium text-koch-100 mb-8">Digital Thread</h2>
            <div className="relative border-l-2 border-border ml-3 space-y-8">
              {timelineEvents.map((event, i) => (
                <div key={i} className="relative pl-8">
                  <div className="absolute -left-[9px] top-1.5 w-4 h-4 rounded-full bg-surface border-2 border-accent"></div>
                  <div className="flex items-center gap-3 mb-1">
                    <span className="font-mono text-sm text-accent">{event.year}</span>
                    <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded bg-surface-overlay text-koch-300 border border-border">
                      {event.type}
                    </span>
                  </div>
                  <h3 className="text-lg font-medium text-koch-50">{event.title}</h3>
                  <p className="text-koch-400 text-sm mt-1 max-w-2xl">{event.desc}</p>
                </div>
              ))}
            </div>
          </TabsContent>
          ...
          <TabsContent value="docs" className="flex-1 min-h-0">
             <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
               {documentCategories.map((cat, i) => (
                 <div key={i} className="bg-surface-elevated border border-border rounded-xl p-6 hover:border-accent/40 transition-colors cursor-pointer group">
                   <div className="w-12 h-12 bg-surface-overlay rounded-lg flex items-center justify-center text-koch-300 group-hover:text-accent group-hover:bg-accent/10 mb-4 transition-colors">
                     <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                       <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d={cat.icon} />
                     </svg>
                   </div>
                   <h3 className="text-lg font-medium text-koch-50 mb-1">{cat.category}</h3>
                   <p className="text-sm text-koch-400 font-mono">{cat.count} Documents</p>
                 </div>
               ))}
             </div>
          </TabsContent>

          <TabsContent value="graph" className="flex-1 min-h-[500px] border border-border rounded-xl overflow-hidden bg-surface-overlay">
            <ReactFlow 
              nodes={initialNodes} 
              edges={initialEdges} 
              fitView 
            ...
            >
              ...
              <Controls className="bg-surface border border-border fill-koch-400" />
            </ReactFlow>
          </TabsContent>

          <TabsContent value="cad" className="flex-1 min-h-[500px] border border-border rounded-xl overflow-hidden bg-surface-overlay">
